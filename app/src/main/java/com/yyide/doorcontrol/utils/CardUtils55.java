...
public class CardUtils55 {
    private static CardUtils55 instance;

    public static CardUtils55 getInstance() {
        if (instance == null)
            instance = new CardUtils55();
        return instance;
    }
    ...
    // 4字节卡号，定制
    private ArrayList<Integer> CC_HEAD = new ArrayList<>();
    // 刷卡数据
    ...
    private CardUtils55() {
        ...
        Collections.addAll(CC_HEAD, 0x55, 0xAA);
    }
    ...
    public String getCardNo(byte[] data, int offset, int length) {
        ...
        // 判断是否存在定制卡的起始标志
        if ((start_index = Collections.lastIndexOfSubList(CD_LIST, CC_HEAD)) >= 0) {
            // 判断4字节卡数据完整性（共7字节，2节包头＋4节卡号＋1节校验）
            if (start_index + 7 <= CD_LIST.size()
                    // 卡数据校验，判断是否匹配校验码
                    && getCheckCode(start_index + 2, start_index + 5) == CD_LIST.get(start_index + 6)) {
                String card_no = convertCardNo(start_index + 2, start_index + 5);
                CD_LIST.clear();
                return card_no;
            }
            // 判断7字节卡数据完整性（共10字节，2节包头＋7节卡号＋1节校验）
            if (start_index + 7 <= CD_LIST.size()
                    // 卡数据校验，判断是否匹配校验码
                    && getCheckCode(start_index + 2, start_index + 8) == CD_LIST.get(start_index + 9)) {
                String card_no = convertCardNo(start_index + 2, start_index + 8);
                CD_LIST.clear();
                return card_no;
            }
        }
